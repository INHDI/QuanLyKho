...
function get_data_dvt() {
    let dropdown, a, j;
    let txtValue = [];
    dropdown = document.getElementById("myDropdown");
    a = dropdown.getElementsByTagName("a");
    for (j = 0; j < a.length; j++) {
        txtValue.push(a[j].textContent || a[j].innerText);
    }
    return txtValue;
}
...
function filterFunction() {
    let input, filter, dropdown, a, i, count = 0, txtValue;
    input = document.getElementById("add_dvt");
    filter = input.value.toUpperCase();
    dropdown = document.getElementById("myDropdown");
    a = dropdown.getElementsByTagName("a");
    let data_dvt = get_data_dvt();
    for (i = 0; i < data_dvt.length; i++) {
        txtValue = data_dvt[i];
...
window.onclick = function (event) {
    if (!event.target.matches('#add_dvt')) {
        ...
        const data_dvt = get_data_dvt();
        if (add_dvt !== "") {
            if (data_dvt.indexOf(add_dvt) === -1) {
